      ...
      "osc": {
        "state": {
          "subscribe": [
            ...
            { "rx1": { "frequency": null, "gain": null, "mute": null, "name": null } },  // Query channel 1 attributes
            { "rx2": { "frequency": null, "gain": null, "mute": null, "name": null } }   // Query channel 2 attributes (adding frequency)
          ],
...
let deviceStates = {};
let clients = [];  // Array to hold connected SSE clients
...
function updateState(device, newState) {
  ...
  // Update channel 1 (TX1)
  if (newState.rx1 || (newState.mates && newState.mates.tx1)) {
    state.channel1.name = newState.rx1 ? newState.rx1.name || state.channel1.name || "N/A" : state.channel1.name;
  ...
  // Update channel 2 (TX2)
  if (newState.rx2 || (newState.mates && newState.mates.tx2)) {
    state.channel2.name = newState.rx2 ? newState.rx2.name || state.channel2.name || "N/A" : state.channel2.name;
  ...
  // Send updated state to all connected SSE clients
  broadcastDeviceStates();
}

function broadcastDeviceStates() {
  clients.forEach(client => {
    client.res.write(`data: ${JSON.stringify(deviceStates)}\n\n`);
  });
}
...
client.on('message', (msg, rinfo) => {
  try {
    ...
    if (device) {
      console.log(`Received message from ${device.name}: ${msg}`);
      updateState(device, response);
    } else {
      console.log(`Received message from unknown device: ${rinfo.address}`);
    }
    ...
    "osc": {
      "state": {
        "subscribe": [
          ...
          { "rx1": { "gain": null } },
          { "rx1": { "mute": null } },
          { "mates": { "tx1": { "battery": { "lifetime": null } } } },
          { "mates": { "tx1": { "mute": null } } },
          { "rx2": { "frequency": null } },
          { "rx2": { "gain": null } },
          { "rx2": { "mute": null } },
          { "mates": { "tx2": { "battery": { "lifetime": null } } } },
          { "mates": { "tx2": { "mute": null } } }
        ],
        "min": 1000,  // Minimum notification period (e.g., 1 second)
        "max": 0      // No maximum period, only send on change
...
app.get('/deviceStates/stream', (req, res) => {
