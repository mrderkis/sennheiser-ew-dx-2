...
function updateState(device, newState) {
  ...
  if (newState.rx1 || (newState.mates && newState.mates.tx1) || (newState.m && newState.m.rx1)) {
    ...
    state.channel1.frequency = newState.rx1 ? (newState.rx1.frequency ? (newState.rx1.frequency / 1000).toFixed(3) + ' MHz' : state.channel1.frequency || "N/A") : state.channel1.frequency;
    ...
    if (newState.mates && newState.mates.tx1 && newState.mates.tx1.mute !== undefined) {
      state.channel1.txMute = newState.mates.tx1.mute;
      console.log(`TX Mute state for ${device.name} Channel 1: ${state.channel1.txMute}`);
    } else {
      state.channel1.txMute = state.channel1.txMute !== undefined ? state.channel1.txMute : "N/A";
    }
    
    // Update RSQI for Channel 1
    if (newState.m && newState.m.rx1 && newState.m.rx1.rsqi !== undefined) {
        state.channel1.rsqi = `${newState.m.rx1.rsqi} %`;
        console.log(`RSQI for ${device.name} Channel 1: ${state.channel1.rsqi}`);
      } else {
        state.channel1.rsqi = state.channel1.rsqi || "N/A";
        console.log(`RSQI data missing for ${device.name} Channel 1.`);
      }
  
      // Update DIVI for Channel 1
      if (newState.m && newState.m.rx1 && newState.m.rx1.divi !== undefined) {
        state.channel1.divi = newState.m.rx1.divi;
        console.log(`Diversity Indicator for ${device.name} Channel 1: ${state.channel1.divi}`);
      } else {
        state.channel1.divi = state.channel1.divi || "N/A";
        console.log(`Diversity Indicator data missing for ${device.name} Channel 1.`);
      }
  ...
  if (newState.rx2 || (newState.mates && newState.mates.tx2) || (newState.m && newState.m.rx2)) {
    ...
    state.channel2.frequency = newState.rx2 ? (newState.rx2.frequency ? (newState.rx2.frequency / 1000).toFixed(3) + ' MHz' : state.channel2.frequency || "N/A") : state.channel2.frequency;
    ...
    if (newState.mates && newState.mates.tx2 && newState.mates.tx2.mute !== undefined) {
      state.channel2.txMute = newState.mates.tx2.mute;
      console.log(`TX Mute state for ${device.name} Channel 2: ${state.channel2.txMute}`);
    } else {
      state.channel2.txMute = state.channel2.txMute !== undefined ? state.channel2.txMute : "N/A";
    }

  // Update RSQI for Channel 2
  if (newState.m && newState.m.rx2 && newState.m.rx2.rsqi !== undefined) {
    state.channel2.rsqi = `${newState.m.rx2.rsqi} %`;
    console.log(`RSQI for ${device.name} Channel 2: ${state.channel2.rsqi}`);
  } else {
    state.channel2.rsqi = state.channel2.rsqi || "N/A";
    console.log(`RSQI data missing for ${device.name} Channel 2.`);
  }

  // Update DIVI for Channel 2
  if (newState.m && newState.m.rx2 && newState.m.rx2.divi !== undefined) {
    state.channel2.divi = newState.m.rx2.divi;
    console.log(`Diversity Indicator for ${device.name} Channel 2: ${state.channel2.divi}`);
  } else {
    state.channel2.divi = state.channel2.divi || "N/A";
    console.log(`Diversity Indicator data missing for ${device.name} Channel 2.`);
  }
...
client.on('message', (msg, rinfo) => {
    try {
        ...
        if (device) {
            console.log(`Received message from ${device.name}:`, JSON.stringify(response, null, 2));
            // Log to check if RSQI and DIVI exist in the response
            if (response.m && response.m.rx1) {
                console.log(`RSQI for RX1: ${response.m.rx1.rsqi}, DIVI for RX1: ${response.m.rx1.divi}`);
            }
            if (response.m && response.m.rx2) {
                console.log(`RSQI for RX2: ${response.m.rx2.rsqi}, DIVI for RX2: ${response.m.rx2.divi}`);
            }
        ...
        "osc": {
            "state": {
                "subscribe": [
                    ...
                    { "m": { "rx1": { "rsqi": null, "divi": null } } },  // Correct Query for RSQI and Diversity Indicator for RX1
                    { "m": { "rx2": { "rsqi": null, "divi": null } } }   // Correct Query for RSQI and Diversity Indicator for RX2
                ],
